...
import pl.bgora.forms.InboxForm;
...
public class InboxAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
			...
			throws Exception {
		InboxForm inboxForm = (InboxForm) form; // NOSONAR;
		User current = (User) request.getSession().getAttribute(
				ActionParams.CURRENT_USER);
		List<Message> messages = messageService.listReceivedMessages(current);
		inboxForm.setMessages(messages);
